...
| `OPENAI_BASE_URL` | Override the OpenAI API origin (default `https://api.openai.com`). May include `/v1`; trailing slashes are trimmed. Used for chat and for **`GET /v1/models`** when populating the TUI model picker against a compatible gateway. |
